...
import {userFetchData} from "../../actions/action"


/* 
  Поле авторизации находится в компоненте Head, данные для проверки вводимых в форму данных
  находятся в data.json, массив users, в actions лежит функция для получения этих данных и проверки,
  если проверка успешная - userData сохраняется в стор, страница по адресу /Admin, доступна только после 
  входа, т к данные кешируются необязательно вводить данные при каждой новой сессии
*/
...
function Head(props) {
  ...
  function handleFormSubmit(event) {
    props.fetchData("./data.json", login, password);
    setOpen(false);
...
const mapDispatchToProps = (dispatch) => ({
  fetchData: (url, login, password) => { 
    dispatch(userFetchData(url, login, password));
  },
});
