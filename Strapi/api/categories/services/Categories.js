...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Categories.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Categories.query(buildQuery({ model: Categories, filters }))
  ...
  count: (params) => {
    ...
    return Categories.query(buildQuery({ model: Categories, filters: _.pick(filters, 'where') })).count();
  },
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = await Categories.forge(params).save(data);
